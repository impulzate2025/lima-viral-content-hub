      ...
      <DialogContent className="sm:max-w-[525px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          <div>
            ...
            <Input id="topic" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Ej: El mejor ceviche de Lima" />
          </div>
          <div>
            <Label htmlFor="audience">Audiencia Objetivo (Obligatorio)</Label>
            <Input id="audience" value={audience} onChange={(e) => setAudience(e.target.value)} placeholder="Ej: Jóvenes universitarios, turistas" />
          </div>
          <div>
            <Label htmlFor="context">Contexto Adicional (Opcional)</Label>
            <Textarea id="context" value={context} onChange={(e) => setContext(e.target.value)} placeholder="Ej: Buscando points baratos, temporada de verano" />
          </div>
          <div>
            <Label htmlFor="viralScoreTarget">Objetivo de Viralidad (1-10)</Label>
            <Input id="viralScoreTarget" type="number" min="1" max="10" value={viralScoreTarget} onChange={(e) => setViralScoreTarget(parseInt(e.target.value, 10))} />
          </div>
